    ...
    <div class="upload-container">
        ...
        <el-dialog  :modal-append-to-body='false' :visible.sync="dialogVisible">
            ...
            <div class="ready-show">
                ...
                >
                    <div class="wrapper" style='z-index:5' v-if='item.showTool'>
                        <!-- 删除图标 -->
                        <i @click='delImg(item,index)'  class="el-icon-delete"></i>
                    </div>
                    <!-- 上传动画 -->
                    ...
                    <div class="img-row" >
                        ...
                        <!-- 上传失败 -->
                        <span class="error" v-if='item.statu === "error"'>上传失败</span>
                    </div>
                </div> 
            </div>  
            <!-- 选取文件 -->
    ...
    methods: {
            ...
            var config = {
                onUploadProgress: progressEvent => {
                    ...
                    if(progressEvent.lengthComputable){
                        ...
                        fileInfo.percentage = progressEvent.loaded / progressEvent.total * 100 | 0
                    }
                }
            }            
        ...
        handleSubmit() {
            ...
            if (!arr.length) {
                this.$message({
                    type: 'warning',
                    message: '请等待所有图片上传成功 或 出现了网络问题 或 当前没选择图片，请刷新页面重新上传!'
                })
                return
            }
            //------ 图片信息传至富文本
        ...
        getFileInfo(e){
            ...
            reader.onload = e => {
                if (typeof e.target.result === "object") {
                    ...
                    fileInfo.url = window.URL.createObjectURL(new Blob([e.target.result]))
