...
<script>
import { createTopic, getMyTopic, getTopicList, getTopicDetails, puTopicDetails } from "@/api/topic.js";
...
export default {
    ...
    created() {
        ...
        if (param.cid) {
            this.cid = param.cid
            this.getTopicDetails(param.cid)
        }
    },
    mounted() {
        this.getMyTopic();
    ...
    methods: {
        ...
        getTopicDetails(id) {
            getTopicDetails(id).then(res => {
                console.log('getTopicDetailsres: ', res)
                if (res.data.data) {
...
.create-circle {
  ...
  .crtcircle-cover {
    ...
    &:hover {
      border: 1px dashed rgba(84, 120, 235, 0.7);
	cursor: pointer;
    }
    .dk-upload {
      width: 130px;
